...
const { getNumber, getString, getBoolean } = require('@lykmapipo/env');
...
exports.defaults = {
  ...
  //see https://github.com/Automattic/kue#creating-jobs
  types: [getString('QUEUE_NAME', 'mongoose')],
...
exports.init = function init(options) {
  ...
  const jobTypes =
    uniq(
      [].concat((exports.defaults || {}).types)
      .concat((exports.options || {}).types)
      .concat((options || {}).types)
    );

  //merge and ensure options
  exports.options = mergeObjects(exports.defaults, exports.options, options);

  //set job types
  exports.options.types = uniq(jobTypes);

  //ensure mongoose connection
  if (mongoose.connection && mongoose.connection !== 1) {
    const MONGODB_URI = exports.options.MONGODB_URI;
    if (!_.isEmpty(MONGODB_URI)) {
      connect(MONGODB_URI);
    }
  }
  ...
  if (!exports.queue) {
    exports.queue =
      (
        ...
        exports.options.queue :
        createQueue(mergeObjects(exports.options))
      );
...
exports.start = function start(options) {
  ...
  if (exports.queue) {
    ...
    jobTypes = _.compact(_.uniq(jobTypes));
