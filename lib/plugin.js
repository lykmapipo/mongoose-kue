...
module.exports = exports = function mongooseKuePlugin(schema, pluginOptions) {
   ...
   * @public
   */
  schema.statics.runInBackground = function runInBackground(optns) {
    ...
    if (model && method) {
      ...
      const job = worker.create(jobType, details).save();
      return job;
    }
   ...
   * @private
   */
  schema.methods.runInBackground = function runInBackground(optns) {
